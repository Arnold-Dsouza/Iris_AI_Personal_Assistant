...
const detectHindiText = (text: string): boolean => {
...
async function tryHuggingFaceAPI(request: ProxyRequestBody): Promise<AIResponse> {
  try {
    ...
    const model = request.language === 'hi-IN' 
      ? 'google/flan-t5-base'  // Model with Hindi support
    ...
    // Validate Hindi responses
    if (request.language === 'hi-IN' && !detectHindiText(aiResponse)) {
      throw new Error('Response does not contain Hindi text');
    }
...
function getLocalizedErrorMessage(language: SupportedLanguage): string {
  switch (language) {
    case 'hi-IN':
      return 'क्षमा करें, सिस्टम वर्तमान में व्यस्त है। कृपया थोड़ी देर बाद पुनः प्रयास करें।';
    default:
